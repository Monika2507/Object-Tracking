function res = vl_simplenn_own(net, x, dzdy, res, varargin)
...
%   l=layers{i};
  ...
  switch l.Name
          ...
          case 'conv'
        ip=double(gather(res(i).x));
        wt=double(l.Weights);
        bs=double(l.Bias);
      res(i+1).x = vl_nnconv(ip,wt,bs,'pad', [2,2,2,2,],'stride', l.Stride,'dilate',1) ;
    case 'conv_1'
        ip=double(gather(res(i).x));
        wt=double(l.Weights);
        bs=double(l.Bias);
     ...
     case 'fc'
        ip=double(gather(res(i).x));
        ip=reshape(ip,[1,size(ip,1)*size(ip,2)*size(ip,3)]);
        wt=double(l.Weights);
        bs=double(l.Bias);
        o=ip.*wt;
        resp=sum(o,2)+bs;
    ...
    case 'convt'
      res(i+1).x = vl_nnconvt(res(i).x, l.weights{1}, l.weights{2}, ...
        ...
        case 'avgpool'
      res(i+1).x = vl_nnpool(res(i).x, l.PoolSize, ...
        'pad', l.PaddingSize, 'stride', l.Stride, ...
        'method', 'max') ;
...
%       if testMode
        ...
        wt=double(l.Scale);
        bs=double(l.Offset);
        res(i+1).x = vl_nnbnorm(ip, wt, bs,'epsilon', l.Epsilon) ;
%       else
